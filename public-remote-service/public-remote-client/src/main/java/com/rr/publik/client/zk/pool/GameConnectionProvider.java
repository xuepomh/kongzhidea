...
public class GameConnectionProvider implements ConnectionProvider {
	...
	public GameConnection getConnection() throws ConnectionException {
		String identify = System.getProperty(GameServiceClientPool.serviceId);
	 ...
	 */
	@Override
	public void updateNodeList(String identify) {
		// 如果是删除操作，则需要 先将当前正在执行的任务执行完毕，才能destory
		for (Node node : nodes) {
			if (node.getIdentity().equalsIgnoreCase(identify)) {
				// 设置not healthy后 轮询时候会跳过改节点
