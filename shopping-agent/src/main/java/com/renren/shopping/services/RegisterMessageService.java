...
public class RegisterMessageService {
	...
	private final Log logger = LogFactory.getLog(RegisterMessageService.class);
...
//		}
	...
	private void saveVerifiCode(String phoneNumber, String code) {
		// 设置一分钟内发过短信
		RedisAPI.setex(
				KVConf.getPhoneRegisterTimeKey(phoneNumber),
				KVConf.REGISTER_TIME, KVConf.PHONE_REGISTER_TIME_VALUE);
		// 存储发送短信验证码
		String codes = RedisAPI.get(
				KVConf.getPhoneRegisterKey(phoneNumber));
		if (StringUtils.isBlank(codes)) {
			codes = code;
		} else {
			codes = codes + "," + code;
		}
		RedisAPI.setex(KVConf.getPhoneRegisterKey(phoneNumber),
				KVConf.REGISTER_CODE_TIME, codes);
	}
}
