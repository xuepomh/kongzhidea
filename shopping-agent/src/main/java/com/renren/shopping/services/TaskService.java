...
public class TaskService {
	 ...
	 * 按条件拼接查询
	 */
	public List<Task> getTaskListByindex2(long endId, int limit, Map<String,String> map){
		String hql;
		if(map.size() == 0){
			hql = "from Task where id < ? and delete_flag = 0 order by id desc";
			return taskDao.getListForPage2(hql, limit, endId);
		}else if(map.size() == 1){
			Set set = map.keySet();
			if(set.iterator().next().equals("goodCountry")){
				hql = "from Task where goodCountry = ? and id < ? and delete_flag = 0 order by id desc";
				return taskDao.getListForPage2(hql, limit, map.get("goodCountry"), endId);
			}else if(set.iterator().next().equals("liveCity")){
				hql = "from Task where liveCity like ? and id < ? and delete_flag = 0 order by id desc";
				return taskDao.getListForPage2(hql, limit, map.get("liveCity") + "%", endId);
			}else{
				if(map.get("gratuity").equals("asc")){
					hql = "from Task where id < ? and delete_flag = 0 order by gratuity asc";
					return taskDao.getListForPage2(hql, limit, endId);
				}else if(map.get("gratuity").equals("desc")){
					hql = "from Task where id < ? and delete_flag = 0 order by gratuity desc";
					return taskDao.getListForPage2(hql, limit, endId);
				}
			}
		}else if(map.size() == 2){
			Set set = map.keySet();
			if(!set.contains("goodCountry")){
				if(map.get("gratuity").equals("asc")){
					hql = "from Task where liveCity like ? and id < ? and delete_flag = 0 order by gratuity asc";
					return taskDao.getListForPage2(hql, limit, map.get("liveCity") + "%", endId);
				}else{
					hql = "from Task where liveCity like ? and id < ? and delete_flag = 0 order by gratuity desc";
					return taskDao.getListForPage2(hql, limit, map.get("liveCity") + "%", endId);
				}
			}else if(!set.contains("liveCity")){
				if(map.get("gratuity").equals("asc")){
					hql = "from Task where goodCountry = ? and id < ? and delete_flag = 0 order by gratuity asc";
					return taskDao.getListForPage2(hql, limit, map.get("goodCountry") , endId);
				}else{
					hql = "from Task where goodCountry = ? and id < ? and delete_flag = 0 order by gratuity desc";
					return taskDao.getListForPage2(hql, limit, map.get("goodCountry") , endId);
				}
			}else{
				hql = "from Task where goodCountry = ? and id < ? and liveCity like ? and delete_flag = 0 order by id desc";
				return taskDao.getListForPage2(hql, limit, map.get("goodCountry") ,endId, map.get("liveCity"));
			}
		}else{
			Set set = map.keySet();
			hql = "from Task where goodCountry = ? and id < ? and liveCity like ? and delete_flag = 0 order by gratuity asc";
			return taskDao.getListForPage2(hql, limit, map.get("goodCountry") ,endId, map.get("liveCity"));
		}
		return null;
	}
	...
	public List<Long> getTaskIds(List<Task> lists){
		List<Long> ids = new ArrayList<Long>();
		for(Task task : lists){
			if(task == null)
				continue;
			ids.add((long) task.getId());
		}
		return ids;
	}
	...
	/**
	 *与上面的getTasksByUser一样
	 * 根据用户id获取用户任务列表
	 */
	public List<Task> getTaskListByUserId(int userId){
		String hql = "from Task where userId = ? and delete_flag = 0";
		return taskDao.find(hql, userId);
	}
	...
	public void supplyJsonObject(JSONObject obj, Task task) {
		...
		try {
			...
			t.put(TaskConf.TASK_STATUS, task.getStatus());
		} catch (Exception e) {
			logger.error(e.getMessage(), e);
			t = new JSONObject();
		}
