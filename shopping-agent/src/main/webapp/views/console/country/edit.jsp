		...
		<div class='main'>
			<div class="content-area">
				...
				<div class='specialtopic-config-area'>
					<form id='edit_form' action='/console/country/editCountry' method='post'>
						<input type="hidden" name="id" value="${country.id }"/>
						<ul class='topic-content'>
							<li class='group'>
								<label class='fm-label'>国家名字</label>
								<input class='fm-input' name='name' value="${country.name }"/>
							</li>
							<li class='group'>
								<label class='fm-label'>国旗 </label>
								<input class='fm-input' name='flag' value="${country.flag }"/>
							</li>
							<li class='group'>
								<label class='fm-label'>国旗-缩略图 </label>
								<input class='fm-input' name='flag_thumb' value="${country.flag_thumb }"/>
							</li>
							<li class='group'>
									...
									<select name='continent' class='fm-select'>
										<c:forEach items="${clist }" var="item">
											<option value="${item.id }"
												<c:if test="${country.continent.id == item.id }">selected="selected"</c:if>
											>${item.name }</option>
										</c:forEach>
									</select>
								</div>
							...
							<li class='group'>
								...
								<input class='fm-input' name='hot' value="${country.hot }" />
							</li>
							
							<li class='group'>
								<label class='fm-label' title="默认是0，按照该值来排序">权重! </label>
								<input class='fm-input' name='weight' value="${country.weight }"/>
							</li>
